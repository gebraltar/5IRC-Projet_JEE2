...
/**
 * Service object for domain model class Niveau.
 * @see .Niveau
 * @author Alexandre Brosse / Hibernate Tools
 */
@Stateless
public class NiveauService implements DaoInterface<Niveau> {

	private static final Log log = LogFactory.getLog(NiveauService.class);
	...
	public void remove(Niveau persistentInstance) {
	...
	public Niveau merge(Niveau detachedInstance) {
		...
		try {
			Niveau result = entityManager.merge(detachedInstance);
	...
	public Niveau findById(Integer id) {
		...
		try {
			Niveau instance = entityManager.find(Niveau.class, id);
